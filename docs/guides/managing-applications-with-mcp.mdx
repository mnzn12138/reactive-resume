...
<Steps>
  <Step title="Connect the MCP server">
    Follow [Using the MCP server](/guides/using-the-mcp-server) to connect your MCP client with OAuth2.
  </Step>
...
```text
List my active applications grouped by stage. Include company, role, tags, follow-up date, linked resume name, and whether a resume PDF is attached.
```
...
Use document attachment when you want the tracker to store the exact resume PDF you sent for an application.
...
```text
Attach this PDF as the sent resume for the Globex Staff Engineer application.
```
...
```text
Remove the resume PDF from the Acme application, but keep the application record and timeline.
```
...
```text
Draft a follow-up message for the Stripe application using the linked resume and job description. Keep it concise and specific to the role.
```
...
<Warning>
  Review AI-generated resumes and messages before sending them. MCP tools can draft and save context, but you are responsible for the final content.
</Warning>
...
```text
Which applications are missing job descriptions, contacts, or sent resume PDFs?
```
...
```text
Use Reactive Resume MCP for application tracking. Start by calling list_applications before creating a new record so you do not duplicate existing applications. Confirm before delete_application, bulk_delete_applications, bulk_update_applications, replacing attached documents, or archiving more than five applications. Prefer add_application_note for timeline updates. Use Application Copilot tools only when the application has enough context, and never present AI-generated follow-ups as sent messages.
```
