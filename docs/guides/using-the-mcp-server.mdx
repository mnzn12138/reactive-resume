...
<Steps>
  <Step title="Sign in to your Reactive Resume account">
    Head over to [https://rxresu.me](https://rxresu.me) (or your self-hosted instance) and sign in, or create an account. The MCP server uses OAuth2, so you authorize each client from your own session.
  </Step>

  <Step title="Make sure your client supports MCP OAuth">
    Reactive Resume authenticates MCP clients with OAuth2 (with dynamic client registration and PKCE). Claude, Cursor, Codex, and other modern clients handle this flow on their own — no manual token handling required.
...
There are two transport options, and both use OAuth2.
...
`mcp-remote` runs the OAuth2 flow for you: the first launch opens a browser window where you sign in and approve access.
...
```json
{
  "mcpServers": {
    "reactive-resume": {
      "command": "npx",
      "args": ["mcp-remote", "https://rxresu.me/mcp"]
    }
  }
}
```

<Info>If the browser window does not open, check that your client can reach <code>https://rxresu.me</code> and that no proxy is blocking the OAuth callback to <code>localhost</code>.</Info>
...
Reactive Resume MCP accepts a single authentication method:

1. **Bearer token (OAuth2 access token)** via `Authorization: Bearer <token>`

If the token is missing or invalid, the MCP endpoint responds with `401` and advertises OAuth metadata using:
...
Cursor then prompts you to sign in with OAuth.
...
Use the `mcp-remote` bridge (example shown above in **Method 2**). It opens a browser window for OAuth2 sign-in on first launch.
...
| `download_resume_pdf`   | Create a 10-minute authenticated PDF download URL for a resume by ID                                                                 |
...
| `attach_application_document` | Attach the sent resume PDF from base64-encoded PDF bytes                                                                        |
| `remove_application_document` | Remove the sent resume PDF                                                                                                     |
...
| `draft_application_message` | Draft a recruiter follow-up from application and resume context                                                                  |
...
| Issue                                                         | Solution                                                                                                                       |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| "Unauthorized" with no login prompt                           | Your client did not start the MCP OAuth flow. Check that it supports remote MCP servers with OAuth, or use the `mcp-remote` bridge |
| OAuth login opens but fails redirect/callback                 | Confirm your client's MCP OAuth callback settings and retry the connection                                                     |
| "API error (401)"                                             | Your access token expired or the client was revoked. Reconnect the server and sign in again                                    |
