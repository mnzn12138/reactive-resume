...
Git can keep owner-controlled versions of your exported resume without changing Reactive Resume or connecting it to a Git provider. This workflow is manual and local only: Reactive Resume does not commit, synchronize, or upload the files for you.

### Choose the right exports

Reactive Resume provides two different JSON shapes:
...
| **Account archive** | Open **Settings**, go to **Account**, and select **Export my data**. | Account profile metadata, all owned resume records, and an `exportedAt` timestamp. | No. It is an archive, not a single-resume import file. |

Use the single-resume export for the Git workflow below. An account archive is an additional portability backup, not a replacement for that file. Because its `exportedAt` timestamp changes on every export, do not expect otherwise unchanged account archives to be byte-for-byte identical.

### Create the local backup

1. Create a folder outside your Reactive Resume source checkout.
2. Export the resume you want to keep.
3. Give the file a stable, descriptive name. The example below uses `resume.json`; replace the file with its latest export instead of changing its name.
...
git diff --cached -- resume.json
git commit -m "Back up resume"
```
...
Only a single-resume JSON export works for this recovery flow. Restoring an account archive wholesale is outside this workflow.
