...
When you connect a tool to the API or the MCP server, you authorize it through OAuth2. The Service stores:

- The registered OAuth client (name, redirect URIs, and the resources it may access)
- OAuth access and refresh tokens for that client, along with your consent records
- Rate limit counters for API and MCP requests

Access and refresh tokens expire, and a connection stops working as soon as its tokens are revoked, expired, or the client is removed.
