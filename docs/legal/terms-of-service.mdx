...
If the Service Operator enables API or MCP access:

- You are responsible for the clients you authorize and for keeping their tokens secure. Revoke or re-authorize a connection if it may be compromised.
- The Service Operator may enforce rate limits, quotas, and permissions on API and MCP requests.
- Misuse of API or MCP access may result in suspension or termination.
