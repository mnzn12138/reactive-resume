...
description: "Automate resume workflows in Reactive Resume with the REST API, the Patch API, the MCP server, AI agent tools, and the JSON resume schema."
---
...
- Attach the resume PDF sent for an application.
- Score a linked resume against a job description and create a tailored resume copy.
- Draft recruiter follow-ups from saved application context.
...
Both the REST API and the MCP server authenticate with OAuth2: clients send an access token as `Authorization: Bearer <token>`. If you self-host, use your own instance URL for the API and MCP endpoints.
...
For scripts and integrations, get an access token with [Using the API](/guides/using-the-api), then use [Using the patch API](/guides/using-the-patch-api) for targeted edits. For agent workflows, start with [Using the MCP server](/guides/using-the-mcp-server).
