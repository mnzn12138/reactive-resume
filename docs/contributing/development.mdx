...
<Info>
  **Prerequisites**: - [Node.js](https://nodejs.org/) v24 - [pnpm](https://pnpm.io/) v12.3.4 -
...
<Steps>
  <Step title="Clone the Repository">
    ```bash
    git clone https://github.com/mnzn12138/reactive-resume.git reactive-resume
    cd reactive-resume
    ```
  </Step>
  ...
  <Step title="Configure Environment Variables">
    ...
    # Email (optional)
    # Leave these unset during development: the app then logs the rendered email to
    # the server console instead of sending it, so nothing reaches a real address.
    # SMTP_HOST=localhost
    # SMTP_PORT=1025
    ...
    <Warning>
      **`.env` vs `.env.local`**: the runtime loads only the repository-root `.env`; it never reads `.env.local`. Running `pnpm dev` without the `dotenvx` prefix therefore ignores `.env.local` entirely. Environment variables that are already set always win over file values.
    </Warning>

    <Tip>
      **Email testing**: SMTP is optional. When `SMTP_HOST`, `SMTP_USER`, `SMTP_PASS`, and `SMTP_FROM` are not all set, the app logs the rendered email to the server console instead of sending it — verification links appear in the dev server output.
    </Tip>

  </Step>
  
  <Step title="Run Database Migrations If Needed">
    The server startup path runs migrations before serving traffic, so this step is only needed when you want to apply
    migrations without starting the app. `packages/db/drizzle.config.ts` loads the repository-root `.env.local` and
    `.env` itself, so no `dotenvx` prefix is required:
...
The console at `/admin` requires an account whose role is `admin`, and accounts created through sign-up start as
regular users. Promote one from the repository root:

```bash
pnpm admin:promote -- you@example.com
```

Pass `--revoke` to remove the role again. When SMTP is not configured, the forgot-password flow cannot send mail, so set
a password directly instead:

```bash
pnpm admin:reset-password -- you@example.com '<new-password>'
```

<Warning>`pnpm db:reset` runs `DROP SCHEMA public CASCADE` and wipes the entire database. It is *not* a password reset.</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="GitHub Repository" icon="github" href="https://github.com/mnzn12138/reactive-resume">
